...
from backtester import quantstats
...
class Strategy(ABC):
    ...
    override methods
    `backtester.backtest.core.backtesting.Strategy.init` and
    `backtester.backtest.core.backtesting.Strategy.next` to define
    your own strategy.
    """
    ...
    def I(self,  # noqa: E743
        ...
        `backtester.backtest.core.backtesting.Strategy.next` much like
        `backtester.backtest.core.backtesting.Strategy.data` is.
        ...
        same length as `backtester.backtest.core.backtesting.Strategy.data`, or
        ...
        `backtester.backtest.core.backtesting.Backtest.plot`.
    ...
    def init(self):
        ...
        Declare indicators (with `backtester.backtest.core.backtesting.Strategy.I`).
        Precompute what needs to be precomputed or can be precomputed
        in a vectorized fashion before the strategy starts.

        If you extend composable strategies from `backtester.backtest.core.backtesting.lib`,
    ...
    def next(self):
        ...
        `backtester.backtest.core.backtesting.Strategy.data`
        instance (row; full candlestick bar) becomes available.
        This is the main method where strategy decisions
        upon data precomputed in `backtester.backtest.core.backtesting.Strategy.init`
        take place.
    ...
    def record(self, name: str = None, plot: bool = True, overlay: bool = None, color: str = None, scatter: bool = False, **kwargs):
        ...
        Args:
            ...
            plot: If True, the time series will be plotted on the resulting `backtester.backtest.core.backtesting.Backtest.plot`.
    ...
    def data(self) -> _Data:
        ...
        `backtester.backtest.core.backtesting.Backtest.__init__`,
        ...
        * Within `backtester.backtest.core.backtesting.Strategy.init`, `data` arrays
          are available in full length, as passed into
          `backtester.backtest.core.backtesting.Backtest.__init__`
          (for precomputing indicators and such). However, within
          `backtester.backtest.core.backtesting.Strategy.next`, `data` arrays are
          only as long as the current iteration, simulating gradual
          price point revelation. In each call of
          `backtester.backtest.core.backtesting.Strategy.next` (iteratively called by
          `backtester.backtest.core.backtesting.Backtest` internally),
    ...
    def position(self, ticker: str = None) -> 'Position':
        """Instance of `backtester.backtest.core.backtesting.Position`.
...
class Position:
    ...
    `backtester.backtest.core.backtesting.Strategy.position` within
    `backtester.backtest.core.backtesting.Strategy.next`.
...
class BarTracker:
    def __init__(self):
        self.bar_times = []  # Stores (bar_number, timestamp), always in increasing order

    def add_bar(self, bar_number, timestamp):
        self.bar_times.append((bar_number, timestamp))

    def find_trade_bar(self, trade_time):
        # Binary search to find the last bar where timestamp <= trade_time
        left, right = 0, len(self.bar_times) - 1
        best_index = -1

        while left <= right:
            mid = (left + right) // 2
            if self.bar_times[mid][1] <= trade_time:
                best_index = mid  # Candidate found
                left = mid + 1  # Search in the right half
            else:
                right = mid - 1  # Search in the left half

        return None if best_index == -1 else self.bar_times[best_index][0]
...
class Order:
    ...
    # class‐level counter for order IDs
    ...
    def __init__(self, broker: '_Broker',
        ...
        # Validate prices (cannot be negative)
        for price_name, price_value in [('limit_price', limit_price), 
                                       ('stop_price', stop_price), 
                                       ('sl_price', sl_price), 
                                       ('tp_price', tp_price)]:
            if price_value is not None and price_value < 0:
                raise ValueError(f"{price_name} cannot be negative")
        ...
        self.__id = next(Order._id_counter)
...
class Trade:
    ...
    # class‐level counter for trade IDs
    _id_counter = itertools.count(1)

    def __init__(self, broker: '_Broker', ticker: str, size: int, entry_price: float, entry_bar, entry_time, tag, reason: Optional['Order'] = None):
        # Validate trade size
        if size == 0:
        ...
        self.__tag = tag
        self.__reason = reason
        self.__trade_id = next(Trade._id_counter)
        # Initialize metadata to store SL/TP prices
        self.__metadata = {
            'SL': None,
            'TP': None
        }
    ...
    def close(self, portion: float = 1., finalize=False):
        ...
        order = Order(self.__broker, self.__ticker, size, parent_trade=self,
                      entry_time=self.__broker.now, tag=self.__tag, close=True)
        if finalize:
            return order
        else:
    ...
    def metadata(self) -> dict:
        """
        Metadata dictionary containing SL/TP prices.
        
        Returns:
            dict: Contains keys 'SL', 'TP'
        """
    ...
    def metadata(self, value: dict):
        ...
        Args:
            value: dict containing any of 'SL', 'TP'
        """
        allowed_keys = {'SL', 'TP'}
...
class _Broker:
    def __init__(self, *, data: _Data, cash, holding, commission, spread, margin, trade_on_close, hedging, exclusive_orders,
                 trade_start_date, lot_size, fail_fast, storage,is_option,
                 equity=None, orders=None, trades=None, positions=None, closed_trades=None, bar_tracker=None):
        ...
        # Handle commission as either callable function or tuple of (fixed, relative)
        ...
        rows = len(data.index)
        cols = len(data.tickers) + 2
        equity_array = np.full((rows, cols), np.nan)
        equity_array[:, 0] = self._cash
        self._equity = equity if equity is not None else equity_array
        self.orders : List[Order] = orders if orders is not None else []
        self.trades: Dict[str, List[Trade]] = trades if trades is not None else {ticker: [] for ticker in self._data.tickers}
        self.positions = positions if positions is not None else {ticker: Position(self, ticker) for ticker in self._data.tickers}
        self.closed_trades = closed_trades or []
        self.bar_tracker = bar_tracker or BarTracker()
            ...
            for ticker, size in self._holding.items():
                if size:
                    self.trades[ticker].append(Trade(self, ticker=ticker, size=size, entry_price=self._data[
                        ticker, 'Close'][self._trade_start_bar], entry_bar=0, entry_time= self._data.index[0],tag='preexisting'))
    ...
    def _commission_func(self, order_size, price):
        """Calculate commission based on fixed and relative components."""
        return self._commission_fixed + max(abs(order_size) * price * self._commission_relative,2)

    def get_state(self):
        # Serialize all relevant attributes of broker state
        state = {
            '_cash': self._cash,
            '_holding': dict(self._holding) if self._holding else {},
            '_commission': self._commission,
            '_spread': self._spread,
            '_leverage': self._leverage,
            '_trade_on_close': self._trade_on_close,
            '_hedging': self._hedging,
            '_exclusive_orders': self._exclusive_orders,
            '_trade_start_date': self._trade_start_date,
            '_lot_size': self._lot_size,
            '_fail_fast': self._fail_fast,
            '_storage': self._storage,
            '_is_option': self._is_option,
            '_equity': self._equity.copy() if self._equity is not None else None,
            'orders': list(self.orders),
            'trades': {ticker: list(trades) for ticker, trades in self.trades.items()},
            'positions': {ticker: Position(self, ticker) for ticker in self._data.tickers},
    ...
    def new_order(self,
        ...
        # ignore any trade actions before trade_start_date
        # if self._trade_start_date and self.now.replace(tzinfo=None) < self._trade_start_date:
        #     print("Current Time:", self.now.replace(tzinfo=None))
        #     print("Trade Start Date:", self._trade_start_date)
        #     return
        ...
        if is_long:
            if not (sl or -np.inf) < (limit or stop or adjusted_price) < (tp or np.inf):
                if self._fail_fast:
                    ...
                    print("Order not executed. Long orders require: ", f"SL ({sl}) < LIMIT ({limit or stop or adjusted_price}) < TP ({tp})")
                    return None
            ...
            if not (tp or -np.inf) < (limit or stop or adjusted_price) < (sl or np.inf):
                if self._fail_fast:
                    ...
                    print("Order not executed. Short orders require: ", f"TP ({tp}) < LIMIT ({limit or stop or adjusted_price}) < SL ({sl})")
                    return None
    ...
    def _adjusted_price(self, ticker: str, size=None, price=None) -> float:
        ...
        spread_adjusted_price = base_price * (1 + copysign(self._spread, size))
        return spread_adjusted_price
    ...
    def finalize(self):
        ...
        for order in final_orders:
            ...
            if trade in self.trades[order.ticker]:
                self._reduce_trade(trade, price, size, time_index, self._data.index[time_index])
        ...
        if equity[0] <= 0:
            if not self._is_option: 
                ...
                for trade in self.all_trades:
                    self._close_trade(trade, self.last_price(trade.ticker), i, self._data.index[i])
                self._cash = 0
                self._equity[i:] = 0
    ...
    def next_live(self):
        ...
        ticker_equity = [self.equity(ticker) for ticker in self._data.tickers]
        equity = [total_equity, *ticker_equity, self.margin_available]
        self._equity = np.vstack([self._equity, equity])
    ...
    def _process_orders(self):
        ...
        for order in list(self.orders): #type = order
            ...
            if order.parent_trade:
                ...
                if trade in self.trades[order.ticker]:
                    self._reduce_trade(trade, price, size, time_index, data.index[time_index])
                    assert order.size != -_prev_size or trade not in self.trades[order.ticker]
                if order in (trade._sl_order, trade._tp_order):
                    assert order.size == -trade.size
                    assert order not in self.orders  # Removed when trade was closed
                    reason = "SL Hit" if order == trade._sl_order else "TP Hit"
                    trade.reason = reason
            ...
            if not self._hedging:
                ...
                for trade in list(self.trades[order.ticker]):
                    ...
                    if abs(need_size) >= abs(trade.size):
                        self._close_trade(trade, price, time_index, data.index[time_index])
                        need_size += trade.size
                    else:
                        # The existing trade is larger than the new order,
                        # so it will only be closed partially
                        self._reduce_trade(trade, price, need_size, time_index, data.index[time_index])
                        need_size = 0
            ...
            # Open a new trade
            if need_size:
                self._open_trade(order.ticker, adjusted_price, need_size, order.sl, order.tp, time_index, entry_time, order.tag)
    ...
    def _reduce_trade(self, trade: Trade, price: float, size: float, time_index: int, time: datetime):
        ...
        self._close_trade(close_trade, price, time_index, time)

    def _close_trade(self, trade: Trade, price: float, time_index: int, exit_time: datetime):
        ...
        self.closed_trades.append(trade._replace(exit_price=price, exit_bar=time_index, exit_time = exit_time ))
        self._cash += trade.pl

    def _open_trade(self, ticker: str, price: float, size: int,
                    sl: Optional[float], tp: Optional[float], time_index: int, entry_time: datetime, tag):
        trade = Trade(self, ticker, size, price, time_index, entry_time, tag)
...
class Backtest:
    ...
    `backtester.backtest.core.backtesting.Backtest.run` to run a backtest
    instance, or `backtester.backtest.core.backtesting.Backtest.optimize` to
    optimize it.
    """
    ...
    def __init__(self,
        ...
        `strategy` is a `backtester.backtest.core.backtesting.Strategy`
        ...
        E.g. set it to `0.0002` for commission-less forex trading where the
        average spread is roughly 0.2‰ of the asking price.
        ...
        # Validate commission parameter - can be Number, callable, or tuple
        if not (isinstance(commission, Number) or callable(commission) or 
                (isinstance(commission, (list, tuple)) and len(commission) == 2)):
            raise TypeError('`commission` must be a Number, callable function, or tuple of (fixed, relative)')
        if not isinstance(spread, Number):
            raise TypeError('`spread` must be a float value, percent of '
                            'entry order price')
        
        # Validate cash and commission parameters
        if cash <= 0:
            raise ValueError("Cash must be positive")
        if isinstance(commission, Number) and commission < 0:
        ...
        missing_columns = set(ohlc) - set(data.columns.levels[1])
        if missing_columns:
        ...
        self._broker = partial(
            _Broker, cash=cash, holding=holding, commission=commission, spread=spread, margin=margin,
    ...
    def run(self, show_progress: bool = False, **kwargs) -> pd.Series:
        ...
        if show_progress:
            pbar = tqdm(total=len(self._data) - start, desc='Running backtest')
        ...
        with np.errstate(invalid='ignore'):
            ...
            if self.trade_start_date is not None:
                equity = equity[equity.index >= self.trade_start_date]
                self._ohlc_ref_data= self._ohlc_ref_data[self._ohlc_ref_data.index>=self.trade_start_date]
    ...
    def run_live(self, **kwargs) -> pd.Series:
        """
        Run the backtest. Returns `pd.Series` with results and statistics.
        ...
        return self._results.copy(), state

    def optimize(self, *,
                 metric: Union[str, Callable[[pd.Series], float], List[Union[str, Callable[[pd.Series], float]]]] = 'SQN',
                 objective: Union[str, List[str]] = 'maximize',
                 method: str = 'grid',
        ...
        `metric` is a string key from the `backtester.backtest.core.backtesting.Backtest.run`-returned results series,
        or a function that accepts this series object and returns a number. By default, the method maximizes
        Van Tharp's [System Quality Number](https://google.com/search?q=System+Quality+Number).

        `objective` determines whether to maximize or minimize the metric.
        Use 'maximize' when higher values of the metric are better (e.g., Sharpe ratio, SQN, returns).
        Use 'minimize' when lower values of the metric are better (e.g., drawdown, volatility, VaR).
        By default, the method maximizes the metric.
        ...
        * `"skopt"` which finds close-to-optimal strategy parameters using
        ...
        metrics_list = list()
        metrics_keys = list()
        metrics_evaluators = list()
        objectives_list = list()

        if isinstance(metric, list):
            metrics_list = metric
            if isinstance(objective, list):
                assert len(objective) == len(metric) # length must match
                objectives_list = objective
            else:
                objectives_list = [objective] * len(metric)
        else:
            metrics_list = [metric] # if metric is not a list than create singleton list
            objectives_list = [objective] # # if objective is not a list than create singleton list
        
        # validating each objective
        for obj in objectives_list:
            if obj not in ['maximize', 'minimize']:
                raise ValueError(f"Objective must be either 'maximize' or 'minimize', got '{obj}'")
        ...
        for i, m in enumerate(metrics_list):
            if isinstance(m, str):
                ...
                if metric_key not in stats:
                    raise ValueError(f"Metric '{metric_key}' must match a key in pd.Series 'result of backtest.run()")
                
                def metric_evaluator(stats: pd.Series, _key=metric_key):
                    return stats[_key]
                
                metrics_evaluators.append(metric_evaluator)

            elif callable(m):
                metric_key = f"metric_{i}"
                metrics_keys.append(metric_key)
                metrics_evaluators.append(m)
            
            else:
                raise ValueError('Each `metric` must be str (a field of backtest.run() result Series) or a function that accepts result Series and returns a number; the higher the better')
        ...
        def _optimize_grid() -> Union[pd.Series, Tuple[pd.Series, pd.Series], Dict, Tuple[Dict, Dict]]:
            ...
            # creating heatmaps for each metric
            heatmaps = {}
            for metric_key in metrics_keys:
                heatmaps[metric_key] = pd.Series(
                    np.nan, 
                    name=metric_key,
            ...
            columns = list(next(iter(param_combos)).keys()) + metrics_keys
            ...
            backtest_uuid = uuid.uuid4()
            param_batches = list(_batch(param_combos))
            
            # store multiple metric evaluators instead of just one
            Backtest._mp_backtests[backtest_uuid] = (self, param_batches, metrics_evaluators)  # type: ignore
            try:
                ...
                if mp.get_start_method(allow_none=False) == 'fork':
                    with ProcessPoolExecutor() as executor:
                        ...
                        for future in _tqdm(as_completed(futures), total=len(futures),
                            ...
                            batch_index, all_values = future.result()
                            for value, params in zip(all_values, param_batches[batch_index]):
                                param_tuple = tuple(params.values())
                                # store results for each metric
                                for metric_idx, val in enumerate(value):
                                    metric_key = metrics_keys[metric_idx]
                                    heatmaps[metric_key][param_tuple] = val
                                ...
                                if live_log_path:
                                    for metric_key in metrics_keys:
                                        sanitized_sheet_name = sanitize_sheet_name(metric_key)
                                        if not os.path.exists(live_log_path):
                                            pd.DataFrame(columns=columns).to_excel(live_log_path, sheet_name=sanitized_sheet_name, index=False)
                                        with pd.ExcelWriter(live_log_path, mode='a', if_sheet_exists='overlay') as writer:
                                            results_df.tail(1).to_excel(writer, sheet_name=sanitized_sheet_name, index=False, header=not os.path.exists(live_log_path))
                else:
                    if os.name == 'posix':
                        warnings.warn("For multiprocessing support in `Backtest.optimize()` "
                                      "set multiprocessing start method to 'fork'.")
                    for batch_index in _tqdm(range(len(param_batches)), disable = True):
                        _, all_values = Backtest._mp_task(backtest_uuid, batch_index)
                        for values, params in zip(all_values, param_batches[batch_index]):
                            param_tuple = tuple(params.values())

                            # store results for each metric
                            for metric_idx, val in enumerate(values):
                                metric_key = metrics_keys[metric_idx]
                                heatmaps[metric_key][param_tuple] = val
            ...
            for i, metric_key in enumerate(metrics_keys):
                ...
                if heatmap.isna().all():
                    # to handle case when all values are in series are NA
                    stats = self.run(**param_combos[0]) # 
                else:
                    if obj == "maximize":
                        best_params = heatmap.idxmax(skipna=True)
                    else: # obj == "minimize"
                        best_params = heatmap.idxmin(skipna=True)

                    stats = self.run(**dict(zip(heatmap.index.names, best_params)))

                best_results[metric_key] = stats
            
            # return results based on when we have single or multiple metrics
            if len(metrics_keys) == 1:
                # backward compatibility for single metric
                if return_heatmap:
                    return best_results[metrics_keys[0]], heatmaps[metrics_keys[0]]
                return best_results[metrics_keys[0]]
            else:
                # return dictionary of results for multiple metrics
                if return_heatmap:
                    return best_results, heatmaps
                return best_results
        ...
        def _optimize_skopt() -> Union[pd.Series,
            ...
            def objective_function(**params):
                ...
                # apply optimize objective
                if objective == 'maximize':
                    value = -metric_evaluator(res) # negating for maximize
                else: # objective == 'minimize'
                    value = metric_evaluator(res) # minimize is by default nature
                if np.isnan(value):
                    return INVALID
                return value
        ...
        def _optimize_sambo() -> Union[pd.Series,
            ...
            # heatmaps for each metrics
            heatmaps = {metric_key: pd.Series(name=metric_key) for metric_key in metrics_keys}
            best_results = {}

            # process each metric separately with its own objective as sambo.minimize() only does one objective at a time
            for metric_idx, (metric_key, objective) in enumerate(zip(metrics_keys, objectives_list)):
                # Avoid recomputing re-evaluations
                @lru_cache()
                def memoized_run(tup):
                    nonlocal metric, self
                    stats = self.run(**dict(tup))
                    value = metrics_evaluators[metric_idx](stats)
                ...
                progress = iter(_tqdm(repeat(None), total=max_tries, leave=False,
                                    desc=f"Optimizing {metric_key}", mininterval=2))
                _names = tuple(kwargs.keys())

                def objective_function(x, *, _progress=progress, _run=memoized_run, _names=_names):
                    next(_progress)
                    value = _run(tuple(zip(_names, x)))
                    return 0 if np.isnan(value) else value

                def cons(x, *, _constraint=constraint, _names=_names):
                    return _constraint(AttrDict(zip(_names, x)))
                ...
                if return_heatmap:
                    # TODO: Implement objective handling for heatmap values
                    # For now, we always maximize by negating the function values
                    # handle the objective direction for heatmap values
                    values = res.funv
                    if objective == "maximize":
                        values = -values # unreverse the negation performed for maximization
                    
                    heatmap_data = {tuple(x): v for x,v in zip(res.xv, values)}
                    heatmaps[metric_key] = pd.Series(heatmap_data, name=metric_key)
                    heatmaps[metric_key].index.names = kwargs.keys()
                    heatmaps[metric_key].sort_index(inplace=True)
            output = list()
            
            # for single metric, backward compatibility
            if len(metrics_keys) == 1:
                metric_key = metrics_keys[0]
                if return_heatmap:
                    output.extend([best_results[metric_key], heatmaps[metric_key]])
                else:
                    output.append(best_results[metric_key])
            else:
                # for multiple metrics
                if return_heatmap:
                    output.extend([best_results, heatmaps])
                else:
                    output.append(best_results) 
            
            if return_optimization:
                output.append(res) # for multiple metrics just return last optimization result  
    ...
    def _mp_task(backtest_uuid, batch_index):
        bt, param_batches, metric_evaluators = Backtest._mp_backtests[backtest_uuid]

        batch_results = list()
        for params in param_batches[batch_index]:
            stats = bt.run(**params)
            if stats['# Trades']:
                # compute all metrics for this parameter combination
                param_results = [evaluator(stats) for evaluator in metric_evaluators]
            else:
                # if no trades, set all metrics to NaN
                param_results = [np.nan for _ in metric_evaluators]
            batch_results.append(param_results)

        return batch_index, batch_results

    _mp_backtests: Dict[uuid.UUID, Tuple['Backtest', List, List[Callable]]] = {}
    ...
    def plot(self, *, results: pd.Series = None, filename=None, plot_width=None,
             ...
             resample=False, reverse_indicators=False,
             show_legend=True, open_browser=True,
             plot_allocation=False, relative_allocation=True,
             plot_indicator=True,plotting_date = None):
        """
        Plot the progression of the last backtest run.

        If `results` is provided, it should be a particular result
        `pd.Series` such as returned by
        `backtester.backtest.core.backtesting.Backtest.run` or
        `backtester.backtest.core.backtesting.Backtest.optimize`, otherwise the last
        ...
        for i in range(len(indicators)):
            indicators[i].index = pd.to_datetime(indicators[i].index)
            if plotting_date is not None:
                indicators[i] = indicators[i][indicators[i].index >= plotting_date]
    ...
    def tear_sheet(self, *, results: pd.Series = None, plotting_date = None, filename=None, open_browser=True, output_path=None, equity_curve=False):
        ...
        if plotting_date is not None:
            ...
            trade_table['EntryTime'] = pd.to_datetime(trade_table['EntryTime'])
            trade_table['ExitTime'] = pd.to_datetime(trade_table['ExitTime'])
            trade_table['EntryBar'] = trade_table['EntryTime'].apply(lambda x: equity_series.index.get_loc(x) if x in equity_series.index else -1)
            trade_table['ExitBar'] = trade_table['ExitTime'].apply(lambda x: equity_series.index.get_loc(x) if x in equity_series.index else -1)
        ...
        quantstats.reports.html(
            equity_series,
            strategy_name=results['strategy_name'] if 'strategy_name' in results else None,
            rm_name=results['rm_name'] if 'rm_name' in results else None,
            tm_name=results['tm_name'] if 'tm_name' in results else None,
            code_id=results['code_id'] if 'code_id' in results else None,
            stock_name=results['stock_name'] if 'stock_name' in results else None,
            timeframe=results['time_frame'] if 'time_frame' in results else None,
            exchange=results['exchange'] if 'exchange' in results else None,
            equity_df=None,
