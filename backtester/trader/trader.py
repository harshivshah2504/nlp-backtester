...
from backtester.backtest.utils import calculate_positions
from backtester.broker import Broker, BrokerAccount
from backtester.datasource import QuoteSource
from backtester.utils.config import config
from backtester.utils.mailjet import mailjet_send_email
from backtester.utils.mtdb import MTDB
from backtester.utils.telegram import send_telegram_message
...
class TradePlan:
    ...
    __storage_root = os.path.expanduser('~/.backtester/storage')
...
class StrategyManager:
    ...
    __strategy_root = os.path.expanduser('~/.backtester/strategy')
...
class BacktestRunner:
    ...
    def execute(self, dryrun: bool = False) -> BacktestLog:
        ...
        try:
            ...
            proc = subprocess.run(
                [sys.executable, '-m', 'backtester.cli', 'backtest', '--run_id', self.run_id, self.plan.id] +
...
class TaskManager:
    ...
    __task_root = os.path.expanduser('~/.backtester/task')
