...
def plot(*, results: pd.Series,
         ...
         plot_indicator=True, plot_sl_tp=True):
    """
    ...
    start_bar = results['_trade_start_bar']
    if len(results['_trades']):
        start_bar = results['_trades'].iloc[0]['EntryBar']
    equity_data = results['_equity_curve'].copy(deep=False)
    equity_data = equity_data.iloc[start_bar:]
    data = data.iloc[start_bar:]
    trades = results['_trades']
    trades['EntryBar']-=start_bar
    trades['ExitBar']-=start_bar
    ...
    baseline = baseline.iloc[start_bar:]
    ...
    trade_source = ColumnDataSource(dict(
        ...
        entry_bar=trades['EntryBar'],
        entry_price=trades['EntryPrice'],
    ))

    # Add SL/TP data to trade_source if available
    if 'SL' in trades.columns and 'TP' in trades.columns:
        # Extract SL/TP data from trade metadata
        sl_prices_list = []
        tp_prices_list = []
        
        for _, trade in trades.iterrows():
            # Extract SL data
            sl_metadata = trade['SL']
            if isinstance(sl_metadata, dict) and 'SL' in sl_metadata:
                sl_prices = sl_metadata['SL']
            else:
                sl_prices = []
            
            # Extract TP data
            tp_metadata = trade['TP']
            if isinstance(tp_metadata, dict) and 'TP' in tp_metadata:
                tp_prices = tp_metadata['TP']
            else:
                tp_prices = []
            
            sl_prices_list.append(sl_prices)
            tp_prices_list.append(tp_prices)
        
        trade_source.add(sl_prices_list, 'sl_prices')
        trade_source.add(tp_prices_list, 'tp_prices')
    else:
        # Add empty lists if no metadata available
        trade_source.add([[] for _ in range(len(trades))], 'sl_prices')
        trade_source.add([[] for _ in range(len(trades))], 'tp_prices')
...
return this.labels[index] || "";
    ...
    def _plot_ohlc_trades():
        ...
        if plot_sl_tp:
            ...
            # grab the raw lists from the ColumnDataSource (or empty lists)
            sl_data = trade_source.data.get('sl_prices', [])
            tp_data = trade_source.data.get('tp_prices', [])

            # iterate with an explicit position counter
            for pos, (idx, trade) in enumerate(trades.iterrows()):
                entry_bar = trade['EntryBar']
                exit_bar  = trade['ExitBar']

                # Safely pull out that trade’s SL/TP (may be float, list or None)
                sl_prices = []
                if pos < len(sl_data):
                    sl_prices = sl_data[pos] or []

                tp_prices = []
                if pos < len(tp_data):
                    tp_prices = tp_data[pos] or []

                # Normalize to a list of numeric prices
                if isinstance(sl_prices, (int, float)) and not np.isnan(sl_prices):
                    sl_prices = [sl_prices]
                elif not isinstance(sl_prices, list):
                    sl_prices = []

                if isinstance(tp_prices, (int, float)) and not np.isnan(tp_prices):
                    tp_prices = [tp_prices]
                elif not isinstance(tp_prices, list):
                    tp_prices = []

                # Build the xs/ys pairs for each SL level
                for price in sl_prices:
                    if isinstance(price, (int, float)) and not np.isnan(price):
                        sl_lines_xs.append([entry_bar, exit_bar])
                        sl_lines_ys.append([price, price])

                # Build the xs/ys pairs for each TP level
                for price in tp_prices:
                    if isinstance(price, (int, float)) and not np.isnan(price):
                        tp_lines_xs.append([entry_bar, exit_bar])
                        tp_lines_ys.append([price, price])
            ...
            if tp_lines_xs:
                fig_ohlc.multi_line(
                    ...
                    line_alpha=0.7,
                    line_dash='dashed',
                    legend_label='Take Profit Levels',
                )
